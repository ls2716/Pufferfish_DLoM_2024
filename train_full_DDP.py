...
def train_ddp(rank, world_size):
    ...
    # create local model
    model = FullRankNet(no_hidden_layers=NO_HIDDEN_LAYERS,
                        hidden_size=HIDDEN_SIZE).to(rank)
    if rank == 0:
        # Print model summary
        logger.info(model)
    ...
    times = train(ddp_model, train_loader, val_loader, epochs=EPOCHS,
                  rank=rank, filename='full_rank_net.pth')
...
if __name__ == '__main__':
    ...
    # Load the full rank model and evaluate the accuracy
